            ...
            using (var host = new Host())
            {
                ...
                while (host.Service(1) >= 0)
                {
                    ...
                    while (host.CheckEvents(out @event) > 0)
                    {
                        ...
                        {
                            case EventType.Connect:
                                Console.WriteLine(" 已连接到server,{0}", @event.ChannelID);
                                //peer = @event.Peer;
                                //for (var i = 0; i < 100; i++)
                                //{
                                //    peer.Send((byte)i, new byte[] { 0, 0 });
                            ...
                            case EventType.Receive:
                                Console.WriteLine(" 正在接收数据", @event.ChannelID);
                                var data = @event.Packet.GetBytes();
                                var value = BitConverter.ToUInt16(data, 0);
                                string ss = System.Text.Encoding.ASCII.GetString(data);
                                //if (value%1000 == 0)
                                //{
                                //    Console.WriteLine("  Client: Ch={0} Recv={1}", @event.ChannelID, value);
                                //}
                                Console.WriteLine("  Client: Ch={0} Recv={1}", @event.ChannelID, ss);
                                value++;
                                peer.Send(@event.ChannelID, BitConverter.GetBytes(value));
                                @event.Packet.Dispose();
                                break;
